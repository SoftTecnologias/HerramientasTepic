$(function () {
 $('#buscar').on('click',function () {
    var filtro = $('#search').val();
     ...
     location.href=document.location.protocol+'//'+document.location.host + '/HerramientasTepic/public' +'/productos/search/'+Base64.encode(filtro);
    ...
    $('.fbrand').on('click',function(){
        if($(this).hasClass('active'))
            removeUrlParameters('marca',$(this).data('val'),uPath[pfinal]);
        else
            toUrlParameters('marca',$(this).data('val'));
    });
    //Filtro Categorias
        ...
        filtros.forEach(function (row, index) {
            ...
            switch (fv[0]) {
                ...
                case 'subcategoria':
                    ...
                    break;
                case 'marca':
                    var marcas = $('ul.brand li');
                    for (var i = 0; i < marcas.length; i++) {
                        indice = fv[1].split(',').indexOf(Base64.decode($(marcas[i]).data('val')));
                        if (indice != -1) {
                            $(marcas[i]).addClass('active');
                            $(marcas[i]).children().addClass('active');
                        }
                    }
                    break;
            }
